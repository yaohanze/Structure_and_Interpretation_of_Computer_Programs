...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        rest_eval = rest.map(lambda x: scheme_eval(x, env))
...
def eval_all(expressions, env):
    ...
    if  expressions is nil:
        ...
        scheme_eval(expressions.first, env) # Handle tail context here.
        return eval_all(expressions.second, env)
    return scheme_eval(expressions.first, env, True) # Handle tail context here.
    # END PROBLEM 8
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if not (formals is nil and vals is nil):
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Wrong number of arguments were passed: {0}.'.format(python_args))
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        define_name = target.first 
        formals = target.second
        body = expressions.second
        env.define(define_name, LambdaProcedure(formals, body, env))
        return define_name
        # END PROBLEM 10
    else:
...
def do_if_form(expressions, env):
    ...
    if scheme_truep(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.second.first, env, True) # Handle tail context here.
    elif len(expressions) == 3:
        return scheme_eval(expressions.second.second.first, env, True) # Handle tail context here.
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.second is nil:
            val = scheme_eval(expressions.first, env, True) # Handle tail context here.
        else:
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.second is nil:
            val = scheme_eval(expressions.first, env, True) # Handle tail cointext here.
        else:
            val = scheme_eval(expressions.first, env)
        if scheme_truep(val):
            return val
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if clause.second is nil:
                ...
                return scheme_eval(clause.second.first, env, True) # Handle tail context here.
            else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        expressions = Pair(scheme_eval(bindings.first.second.first, env), expressions)
        bindings = bindings.second
    check_formals(formals)
    return Frame.make_child_frame(env, formals, expressions)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        formals = target.second
        body = expressions.second
        env.define(define_name, MacroProcedure(formals, body, env))
        return define_name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
